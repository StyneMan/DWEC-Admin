...
const EditDeliveryAgency = () => {
    ...
    uploadTask.on(
      ...
      () => {
        ...
        getDownloadURL(uploadTask.snapshot.ref).then(async (downloadURL) => {
          const mRef = doc(db, "delivery-agency", `${id}`);
          try {
  ...
  const updateAgency = async (e) => {
    ...
    if (!previewPassport) {
      ...
      const fileRef = ref(storage, "delivery-agency/" + id);
  ...
  return (
    <div>
      ...
      <ValidatorForm onSubmit={updateAgency}>
        <Box
          width={"100%"}
          display="flex"
          flexDirection="row"
...
export default EditDeliveryAgency;
